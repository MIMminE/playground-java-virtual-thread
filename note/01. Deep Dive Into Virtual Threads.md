...
`ForkJoinPool`를 포함한 여러 종류의 스레드 풀들은 `Core Pool Size`와 `Maximum Pool Size`라는 개념이 있는데, `Core Pool Size`는 사용하지 않더라도 미리 준비해놓는 스레드의 개수를 말하며 `ForkJoinPool`의 경우에는 이 크기가 시스템 프로세서 개수가 되는 것이다. 
...
선점형 스케줄링은 특정 사용 시간 또는 우선 순위에 따라 실행중에 있는 스레들의 작업을 중간에 멈추고 어디까지 작업했는지에 대한 정보를 저장 후 다음에 실행할 스레드의 정보들을 가져오는 컨텍스트 스위칭이 자연스럽게 많이 발생하게 된다. 
